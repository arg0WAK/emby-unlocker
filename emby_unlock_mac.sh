...
echo "${CYAN}[ℹ]${RESET} Waiting last user privileges..."
sleep 2
ERROR_OUTPUT=$(sudo security add-trusted-cert -d -r trustRoot -k /Library/Keychains/System.keychain "$TEMP_CERT" 2>&1)

if [ $? -eq 0 ]; then
    echo "${GREEN}[✔]${RESET} Certificate successfully added to system store."
else
    echo "${RED}[✖]${RESET} $ERROR_OUTPUT"
    exit 1
fi
...
echo "${YELLOW}[ℹ]${RESET} Restarting network services..."
sleep 2
if sudo dscacheutil -flushcache && sudo killall -HUP mDNSResponder; then
    echo "${GREEN}[✔]${RESET} Network services restarted successfully."
else
    echo "${RED}[✖]${RESET} Failed to restart network services. Please check your system settings."
    exit 1
fi
